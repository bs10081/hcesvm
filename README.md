...
- **Hierarchical Classifier**: 層次分類器
  - 支援 3 類固定/動態策略與 N 類 `test3` / `test4` 級聯結構
  - 目前有效策略：`single_filter`, `multiple_filter`, `inverted`, `test3`, `test4`
...
hc = HierarchicalCESVM(
    ...
    strategy='multiple_filter'  # 可選: single_filter, multiple_filter, inverted, test3, test4
)
...
hc_imbalanced = HierarchicalCESVM(
    ...
    strategy='test4'  # test3 分組 + test4 目標函數
)

# 訓練（test4 會使用 normalized indicator penalty 與 balanced accuracy terms）
...
`HierarchicalCESVM(strategy=...)` 目前支援 **5 種策略**：
...
與 `test4` 支援 3 類與 N 類資料，會建立 `N-1` 個 binary classifiers。
`test3` 使用 `accuracy_mode="both"` 與 `class_weight="balanced"`；`test4`
沿用 `test3` 分組，並使用 `objective_variant="test4"`。
...
| 3-class 或 N-class teaching-data | `test4` | test3 分組 + normalized indicator penalty |
...
`test3` 使用原本第三版目標函數，accuracy lower-bound terms 依正/負類樣本數倒數加權：
...
`test4` 是本次新模型：沿用 `test3` 分組，但 indicator penalty 依該 binary
classifier 的 training sample size 正規化，accuracy lower-bound terms 維持
正/負類樣本數倒數加權：
...
- 4-class `test3` / `test4` 會訓練 3 個 hyperplanes；若 `time_limit=1800`，最壞情況約為
  `1800s * 3`，因為目前是串行訓練。
...
- `objective_variant` (str): `standard` 或 `test4`；`HierarchicalCESVM(strategy="test4")` 會自動設定
...
- `strategy` (str): `single_filter`, `multiple_filter`, `inverted`, `test3`, `test4`
- `n_classes` (int, optional): 類別數；N-class 目前支援 `test3` 與 `test4`
