...
- 策略名稱使用目前 runner 或實驗名稱，例如 `single_filter`, `multiple_filter`,
  `inverted`, `test3`, `test3_teaching_data_1000`, `test3_no_time_limit_full`
...
`HierarchicalCESVM(strategy=...)` 目前接受 4 種策略：
...
- 4-class `test3` 會訓練 3 個 classifiers；若 `time_limit=1800`，最壞情況約為
  `1800s * 3`，因為目前是串行訓練。
