#!/usr/bin/env python3
"""Test only the previously failed datasets (New_Thyroid, Squash_Stored, Squash_Unstored)."""
...
# Only test the 3 failed datasets
DATASETS = [
...
def main():
    """Run only the 3 failed datasets."""
    print("=" * 80)
    print("Testing Previously Failed Datasets (Inverted Strategy)")
    print("=" * 80)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    print("Datasets to test (previously failed):")
    ...
    print("Estimated total time: 1-3 hours")
    print("=" * 80)
    print()
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    summary_file = results_dir / f"inverted_failed_datasets_summary_{timestamp}.md"

    with open(summary_file, 'w') as f:
        f.write("# Inverted Strategy - Failed Datasets Retest Summary\n\n")
